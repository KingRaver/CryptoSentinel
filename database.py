...
class CryptoDatabase:
    ...
    def _initialize_database(self):
        ...
        try:
            ...
            # KAITO Layer1 Comparison Table (for backward compatibility)
            cursor.execute("""
            ...
            # NEW: Token Market Comparison Table
            cursor.execute("""
            ...
            # New indices for token-specific tables
    ...
    def store_correlation_analysis(self, analysis: Dict[str, float]) -> None:
        """Store correlation analysis results (for backward compatibility)"""
        conn, cursor = self._get_connection()
        try:
    ...
    def store_kaito_layer1_comparison(self, comparison_data: Dict[str, Any]) -> None:
        """Store KAITO vs Layer 1 comparison data (for backward compatibility)"""
        conn, cursor = self._get_connection()
        try:
    ...
    def get_kaito_vs_layer1_stats(self, hours: int = 24) -> Dict[str, Any]:
        """Get statistical summary of KAITO vs Layer 1 performance (for backward compatibility)"""
        conn, cursor = self._get_connection()
        try:
            cursor.execute("""
                SELECT 
